...
class PositionEmbedding(nn.Module):  # paper page11 B.2
    ...
    def forward(self, x):
        lat_base = (
            x[:, :, 0:1] * torch.Tensor(self.lat_coeff)
        ) 
        lon_base = x[:, :, 1:2] * torch.Tensor(self.lon_coeff)
        depth_base = x[:, :, 2:3] * torch.Tensor(self.depth_coeff)
        ...
        index = (
            (maskk.unsqueeze(0).unsqueeze(0)).expand(x.shape[0], 1, self.emb_dim)
        )
...
class full_model(nn.Module):
    ...
    def forward(self, data):
        ...
        emb_output = self.model_Position(
            torch.DoubleTensor(data["sta"].reshape(-1, 1, data["sta"].shape[2])).float()
        )
        ...
        pga_pos_emb_output = self.model_Position(
            torch.DoubleTensor(data["target"].reshape(-1, 1, data["target"].shape[2])).float()
        )
        ...
        pad_mask = torch.cat((station_pad_mask, target_pad_mask), dim=1)
        ...
        mlp_input = transformer_output[:, -self.pga_targets :, :]
